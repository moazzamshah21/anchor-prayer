...
import GuidedPrayerService from '../services/GuidedPrayer/GuidedPrayerService';
const {width} = Dimensions.get('window');

const GuidedPrayerItem = ({item, index, navigation, onPressPray}) => {
  ...
  const LikeAndDislikePrayer = async (id, value) => {
    ...
    if (value == false) {
      var response = await GuidedPrayerService.LikePrayer(payload);
      if (response?.success) {
        setIsHearted(true);
      } else {
        showMessage({
      ...
      var response = await GuidedPrayerService.DisLikePrayer(payload);
      if (response?.success) {
        setIsHearted(false);
      } else {
        showMessage({
  ...
  const addComment = async () => {
    ...
    var response = await GuidedPrayerService.AddComment(payload);
    if (response?.success) {
      ...
      <View style={{justifyContent: 'center', flexGrow :1}}>
        ...
        <View style={{flexDirection: 'row', alignItems: 'center'}}>
          {/* <Image
            ...
            resizeMode="contain"
          /> */}
          <Text style={styles.ViewedText} numberOfLines={2}>
            {item?.description}
          </Text>
        </View>
      </View>
...
export default GuidedPrayerItem;
